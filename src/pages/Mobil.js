...
import { getNumberInFirstString } from "../helper/others";

const Mobil = () => {
    let { title } = useParams();
    const [id, setId] = useState(getNumberInFirstString(title));
    const [car, setCar] = useState(getDetailCar(getNumberInFirstString(title)));
    ...
    useEffect(() => {
        setId(getNumberInFirstString(title));
        setCar(getDetailCar(getNumberInFirstString(title)));
        getCar();
    }, []);
